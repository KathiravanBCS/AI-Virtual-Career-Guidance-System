...
import type { Settings, ShowForm } from '@/lib/store/useSettingsStore';
import { DEFAULT_FONT_COLOR } from '@/lib/store/useSettingsStore';
...
export const ResumePDF = ({
  ...
  const { profile, summary, workExperiences, educations, projects, skills, custom } = resume;
  const { name } = profile;
  ...
  const formTypeToComponent: { [type in ShowForm]: () => React.ReactElement } = {
    ...
    custom: () => (
      <ResumePDFCustom custom={custom} themeColor={themeColor} showBulletPoints={showBulletPoints['custom']} />
    ),
  };
