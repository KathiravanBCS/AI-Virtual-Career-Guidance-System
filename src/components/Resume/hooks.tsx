...
export const useSetDefaultScale = ({
  ...
  useEffect(() => {
    const getDefaultScale = () => {
      ...
      const topNavBarHeightRem = 3.5; // --top-nav-bar-height
      const resumeControlBarHeight = 3; // --resume-control-bar-height
      const resumePadding = 1.5; // --resume-padding
      const topAndBottomResumePadding = resumePadding * 2;
      const defaultResumeHeightRem =
        screenHeightRem - topNavBarHeightRem - resumeControlBarHeight - topAndBottomResumePadding;
